...
First, in treating crystalline solids one imposes periodic boundary conditions to reflect the periodicity of the lattice and the Bloch functions. With periodic boundary conditions, the position operator, that is not periodic, is ill-defined. Then, a definition of the dipole operator, and thus of the polarizability,  that relies on the position is ill-defined.% in periodic systems, because $\vec \rr$ is not periodic while wave-functions are. WF ARE NOT PERIODIC... BLOCH THEO
...
\bea
\HH_{mb} (t) &=& \hh_{\kk} + \Delta \hh_{\kk} + \UU_{\kk} +\VV_{\kk}^H[n-n_0] + \SiS_{\kk}^{\text {sex}}[\rho]- \SiS_{\kk}^{\text {sex}}[\rho_0].
\label{mbhamiltonian}
\eea
...
A real-time simulation outputs the real-item polarisation. Similarly to an experiment, we can change the intensity and temporal behavior of the external field and the signal post-processing to obtain the response function of interest.      
