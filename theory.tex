...
\label{chapterberry}
The EOMs for density matrix EOMs, Eq.~\ref{eomsimple}], have been very useful to derive analitic expression for the response function at different orders, however their application in the real-time for the non-linear response has been limited due to the difficulties to describe the coupling between light and electron in periodic system. Here we will present an alternativeformalisme based on an effetive Schr\"odinger which overcomes these difficulties.
