...
\ee                              
In finite systems we impose the condition  $\PP(\rr) \rightarrow 0 $ outside the sample (Dirichlet boundary condition) and $\int n(\rr) d\rr=0$. {\color{red}DS: How is this useful?} \\
In periodic systems, it is most useful to resolve the previous equation into Fourier components $\qq+ \GG$,
...
{\color{red} Maybe here we want to specify that we include only correlations which lead to a renormalization of the hamiltonian $H_A$. No lifetimes or similar.} \\
...
{\color{red} Do we want to distinguish the density matrix $\rho$ from the density $n$? I see later you use $\gamma$ and $\rho$. In case we go for the latter we should change the EOM at the beginning.} 
\be
...
{\color{blue}Approach like the one discussed in Ref.~\onlinecite{nloptics2013} are exact for non interacting electrons. A proper formulation of dephasing beyond the IPA is instead far from trivial within single particle approaches (either density matrix or wave-function based).}
...
{\color{red} Wasn't this shown before in the GGG paper?} \\
...
{\color{red} What about adding a comment on open issues? \\
One is the inclusion of dynamical correlations, i.e. dephasing mechanisms. The core of this highlight is the ability to include static correlations (like excitonic effects). It is an open issue how to properly include dynamical effects and dissipation (liek exc-ph). \\
Another open issue is about static correlations beyond eh interactions, like bi-excitons, trions and similar.}
