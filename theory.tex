...
\label{chapterberry}
We have seen in the previous section that the density matrix EOMs, Eq.~\ref{eomsimple}], allows one to derive analitic expression for the response function at different orders. We have also discussed real-time approaches that integrate directly Eq.~\ref{eomsimple}]. However, within the density matrix formalism describing the coupling between light and electrons in a periodic system is far from straightforward. Here, we present an alternative formalism based on an effetive Schr\"odinger where the coupling between light and electrons in a periodic system is derived from a Berry's phase polarisation. 
