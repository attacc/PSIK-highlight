...
{\color{green} The problem is the same, if it was possible to define dipoles in periodic systems, one could have definted the polarization, this is done for example in the Wannier rappresentation where one can define the polarization in term of localized objects, similar to Clasius-Massotti, for isolated system G=0 is not zero, because one impose the zero of the density at inifite distance and therefore the dipole and polarization are well defined}
...
\bea
\HH_{mb} (t) &=& \hh_{\kk} + \Delta \hh_{\kk} + \UU_{\kk} +\VV_{\kk}^H[n-n_0]+ \SiS_{\kk}^{\text {cohsex}}[\gamma-\gamma_0].
\label{mbhamiltonian}
\eea
...
{\color{green}  May be yes, but the paper of Maitra is so simple and clear that I want to cite it, we can add a second citation} 
